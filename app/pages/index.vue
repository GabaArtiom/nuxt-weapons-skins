<template>
  <div class="min-h-screen tactical-grid-bg">
    ...
    <!-- Modal for knife/glove editing -->
    <Transition name="modal">
      <div v-if="showModal" class="modal-backdrop" @click.self="showModal = false">
        <div class="modal" v-if="modalSkin" :style="{ '--rarity-color': modalSkin.rarity.color }">
          ...
          <div class="modal__head">
            <div class="modal__rarity">
              ...
              {{ modalSkin.rarity.name }}
            </div>
            <h2 class="modal__title">{{ modalSkin.name }}</h2>
          </div>

          <div class="modal__preview">
            <div class="modal__preview-glow"></div>
            <img :src="modalSkin.image" :alt="modalSkin.name" class="modal__preview-img" />
          ...
          <div class="modal__body">
            <FloatSlider
              v-model="floatValue"
              :min="modalSkin.min_float"
              :max="modalSkin.max_float"
            />
            ...
            <label v-if="isKnifeModal" class="stattrak-toggle">
...
// Modal state for knife/glove editing
const showModal = ref(false)
const modalSkin = ref<Skin | null>(null)
const modalPlayerSkin = ref<any>(null)
const modalWeaponDefindex = ref<number | null>(null)
const floatValue = ref(0.01)
const seedValue = ref(0)
const nametag = ref('')
const statTrak = ref(false)
const saving = ref<number | false>(false)
const isKnifeModal = ref(false)

const openKnifeModal = () => {
  if (!activeKnifeSkin.value || !activeKnifePlayerSkin.value) return

  modalSkin.value = activeKnifeSkin.value
  modalPlayerSkin.value = activeKnifePlayerSkin.value
  modalWeaponDefindex.value = activeKnifePlayerSkin.value.weapon_defindex
  floatValue.value = activeKnifePlayerSkin.value.weapon_wear ?? 0.01
  seedValue.value = activeKnifePlayerSkin.value.weapon_seed ?? 0
  nametag.value = activeKnifePlayerSkin.value.weapon_nametag ?? ''
  statTrak.value = !!activeKnifePlayerSkin.value.weapon_stattrak
  isKnifeModal.value = true
  showModal.value = true
}

const openGloveModal = () => {
  if (!activeGloveSkin.value || !activeGlovePlayerSkin.value) return

  modalSkin.value = activeGloveSkin.value
  modalPlayerSkin.value = activeGlovePlayerSkin.value
  modalWeaponDefindex.value = activeGlovePlayerSkin.value.weapon_defindex
  floatValue.value = activeGlovePlayerSkin.value.weapon_wear ?? 0.01
  seedValue.value = activeGlovePlayerSkin.value.weapon_seed ?? 0
  nametag.value = activeGlovePlayerSkin.value.weapon_nametag ?? ''
  statTrak.value = false // Gloves don't have StatTrak
  isKnifeModal.value = false
  showModal.value = true
}

const { saveSkin } = usePlayerSkins()

const saveModalSkin = async (team: number) => {
  if (!modalSkin.value || !modalWeaponDefindex.value) return

  saving.value = team
  try {
    await saveSkin({
      weapon_defindex: modalWeaponDefindex.value,
      weapon_paint_id: parseInt(modalSkin.value.paint_index),
      weapon_wear: floatValue.value,
      weapon_seed: seedValue.value,
      weapon_nametag: nametag.value || null,
      weapon_stattrak: isKnifeModal.value ? statTrak.value : false,
      weapon_team: team,
    })
